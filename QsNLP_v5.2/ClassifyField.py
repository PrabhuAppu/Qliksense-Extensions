...
def checkCardinality(field):
    ...
    result = queryQlikEngine.GetInfo("D:\Qlik\Apps\Executive Dashboard.qvf", request)
...
def doClassify(result, MatchPercent):
        ...
        for selected in finalResult:
            if selected['name']==fieldTags['fieldName']:
                ...
                if fieldTags['fieldName'] in masterDef or fieldTags['isText']=='True' or fieldTags['isTimeStamp']=='True' or fieldTags['isDate']=='True' or fieldTags['isKey']=='True' or (fieldTags['isInteger']=='True' and fieldTags['isNumeric']=='True'):
                    ...
                    if checkCardinality(fieldTags['fieldName'])==True or (fieldTags['isInteger']=='False' and fieldTags['isNumeric']=='True'):
    ...
    print([obj['name'] for obj in finalResult if obj['classification']=='dimension' and obj['percentMatch']>=MatchPercent])

    print('#########-Measure-##########')   
    measure =[obj for obj in finalResult if obj['classification']=='measure'and obj['percentMatch']>=MatchPercent]  
    print([obj['name'] for obj in finalResult if obj['classification']=='measure' and obj['percentMatch']>=MatchPercent])    
