!function(){
//alert(colors('Comp Off'));
//alert(colors('Comp Off'));
	var bP={};	
	//alert((parseInt($(".charts").height())-10));
	var b=30, 
	bb=150, 
	height=390, 
	buffMargin=1, 
	minHeight=14;
	var c1=[-130, 40], c2=[-50, 130], c3=[-10, 140]; //Column positions of labels.
	//var colors =["#2980b9",'#8e44ad','#16a085'];//, "#DC3912",  "#FF9900","#109618", "#990099", "#0099C6"];
	//alert(JSON.stringify(colorbrewer.YlGn[3]));
	//var colors = globalcolor;//colorbrewer.Set1[8];
	//var colors =["rgb(31, 119, 180)", "rgb(214, 39, 40)",  "rgb(255, 127, 14)","rgb(44, 160, 44)", "#990099", "#0099C6"];
	...
	function drawPart(data, id, p){
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			//console.log(data.keys[1][d.key2]);
			return colors(data.keys[1][d.key2]);//d.key2);
	...
	function drawEdges(data, id){
		...
		d3.select("#"+id).select(".edges").selectAll(".edge")
			...
			//return colors(d.key2);
			return colors(data.keys[1][d.key2]);
			})
	...
	function drawHeader(header, id){
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text("# Days").attr("x", (c2[d]-10))
				.attr("y", -5).style("fill","grey");
	...
	bP.draw = function(data, svg){
		var part = parseInt($(".charts").width())/3;
		var partC = parseInt($(".charts").width())/3/6;
		height = parseInt($(".charts").height())-40;
		b=partC;//30;
		bb=partC*4;//150;
		buffMargin=1;
		minHeight=14;
		c1=[-part+20,partC+20];//[-((part/2)+part), b+5];
		c2=[-(part/2)+(part/4),partC+part/2+part/4];//[-(part/2), b+5+(part)]; 
		c3=[-15, part];
		data.forEach(function(biP,s){
			...
			[0,1].forEach(function(p){			
				d3.select("#"+biP.id)
					...
					.select(".mainbars")
					.selectAll(".mainbar")
					.on("mouseover",function(d, i){ console.log('data',data);console.log('p', p); console.log('i', i); return bP.selectSegment(data, p, i); })
					.on("mouseout",function(d, i){ return bP.deSelectSegment(data, p, i); });	
			});
		});
		
		BiPartDataChanged = data;
		//console.log('asas',BiPartDataSelection);
